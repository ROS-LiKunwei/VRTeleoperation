...
6. Oculus VR detector 读取手部数据，并通过 ZeroMQ 发布 keypoints。
7. Transform 组件将原始 keypoints 转换成 VR/world 坐标系下稳定的 6DoF 手部坐标系（位姿），并再次发布。
8. Operator（如 `XArm7RightOperator`）接收该“变换后的手部位姿”，结合标定得到的变换关系计算目标末端执行器（EE）位姿，可选做滤波，然后发布控制命令。
9. Robot interface（如 `XArm7Robot`）订阅命令并调用底层控制器（`DexArmControl`）驱动真实硬件运动，同时发布机器人状态供可视化/记录器订阅。
10. 轻量的 handshake/ACK 机制用于在 publisher/subscriber 之间协调关键状态切换（如 pause/reset），避免竞争条件。
...
- `DexArmControl` (controller)
  - 这是一个围绕XArm硬件的低级API封装。它提供了`XArm7Robot`调用的运动原语（home、move\_cartesian、set modes、query states）.
