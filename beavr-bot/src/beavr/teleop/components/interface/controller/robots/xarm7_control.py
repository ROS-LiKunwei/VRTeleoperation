...
class Robot:
    ...
    def get_arm_position(self):
        ...
        if isinstance(joint_state, (list, tuple, np.ndarray)) and len(joint_state) == 7:
            ...
            print(
                "\033[93m"
                + f"Warning: Unexpected joint state format from get_servo_angle(): {joint_state}"
                + "\033[0m"
            )
            return None
    ...
    def _exceeds_movement_limit(self, target_pose):
        ...
        if distance > self._max_step_distance:
            ...
            print(
                f"\033[93mMovement distance {distance:.2f}mm exceeds {self._max_step_distance}mm limit. "
    ...
    def arm_control(self, cartesian_pose):
        ...
        if exceeds_limit:
            ...
            for waypoint in waypoints:
                ...
                self.set_servo_cartesian_aa(waypoint, wait=True, relative=False, mvacc=5, speed=1)
        else:
            # Execute single movement as before
            self._ensure_correct_mode(1, 0)
            self.set_servo_cartesian_aa(cartesian_pose, wait=False, relative=False, mvacc=5, speed=1)
        ...
        try:
            ...
            if self._last_position is not None:
                position_delta = np.linalg.norm(
                    np.array(cartesian_pos[0:3]) - np.array(self._last_position[0:3])
                )
